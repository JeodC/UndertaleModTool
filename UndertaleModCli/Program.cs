...
public partial class Program : IScriptInterface
{
    ...
    public static int Main(string[] args)
    {
        ...
        var externalizeTexturesOption = new Option<string[]>("-e")
        {
            Description = "Externalize textures (source destination)",
        ...
        dumpCommand.SetAction(parseResult =>
        {
            return Dump(new DumpOptions()
            {
                ...
                ExternalizeTextures = parseResult.GetValue(externalizeTexturesOption)
            });
        });
    ...
    GMImage GetPlaceholderTexture(int _id)   
    {
        ...
        GMImage img = new GMImage(2, 1); 

        // Use Reflection to write to the private _data field
        var field = typeof(GMImage).GetField("_data", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        if (field != null)
        {
            field.SetValue(img, data);
        }
        else
        {
            // Fallback: If the field name is different in this fork, 
            // try to find any byte array field and set it.
            throw new Exception("Could not find internal GMImage buffer to write placeholder data.");
        }
